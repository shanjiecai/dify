        ...
        while True:
            try:
                ...
                if message is None:
                    ...
                    break
                # logger.info(f"[invoke_llm_result]: {message.event}")
                if isinstance(message.event, QueueLLMChunkEvent):
                    message.event = cast(QueueLLMChunkEvent, message.event)
                    message_all += message.event.chunk.delta.message.content
                yield message
            except queue.Empty:
                continue
            finally:
