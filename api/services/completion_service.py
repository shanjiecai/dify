...
class CompletionService:
    ...
    def completion(
        ...
        auto_generate_name = args.get("auto_generate_name", True) if "auto_generate_name" in args else True
        ...
        if conversation_id:
            ...
            if not conversation.override_model_configs:
                ...
                app_model_config = (
                    ...
                    .filter(AppModelConfig.app_id == app_model.id, AppModelConfig.id == app_model.app_model_config_id)
            ...
            if is_model_config_override:
                ...
                model_config = AppModelConfigService.validate_configuration(
                    tenant_id=app_model.tenant_id, account=user, config=args["model_config"], app_mode=app_model.mode
                )
        ...
        file_objs = message_file_parser.validate_and_transform_files_arg(files, app_model_config, user)
    ...
    def generate_more_like_this(
        ...
        # parse files
        message_file_parser = MessageFileParser(tenant_id=app_model.tenant_id, app_id=app_model.id)
        file_objs = message_file_parser.transform_message_files(message.files, app_model_config)
    ...
    def get_message_end_data(cls, data: dict):
        response_data = {"event": "message_end", "task_id": data.get("task_id"), "id": data.get("message_id")}
