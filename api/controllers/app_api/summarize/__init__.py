...
class SummarizeApi(AppApiResource):
    def post(self, app_model: App):
        ...
        if args.get("group_id", None):
            recent_history = get_recent_history_within_timestamp(
                group_id=args["group_id"], start_timestamp=args["start_timestamp"], end_timestamp=args["end_timestamp"]
            )
        ...
        try:
            ...
            if system_prompt == conversation_summary_system_prompt:
                ...
                system_prompt = ""
            response = generate_response(query, system_prompt, **kwargs)
            # 提取出summary和tags
            try:
                summary = (
                    response.choices[0]
                    .message.content.split("Title:")[0]
                    .strip()
                    .split("Summary:")[1]
                    .replace("\n\n-", "")
                    .strip()
            ...
            if args["type"] == "plan":
                ...
                try:
                    nouns = (
                        response.choices[0]
                        .message.content.split("Keywords:")[1]
                        .strip()
                        .split("Title:")[0]
                        .strip()
                        .split(",")
                ...
                for n in nouns:
                    if (
                        n in ["I", "i", "you", "You", "He", "he", "She", "she", "It", "it", "We", "we", "They", "they"]
                        or "dj bot" in n.lower()
                        or "djbot" in n.lower()
                    ):
                        nouns.remove(n)
                return {
