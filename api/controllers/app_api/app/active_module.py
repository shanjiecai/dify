...
def model_chat(conversation_id: str, outer_memory: List, is_force=False):
    ...
    if judge_result:
        ...
        logger.info(f"主动发起聊天：{app_model.name} {conversation_id} {outer_memory[:max(5, len(outer_memory))]}")
...
# 主动询问是否回话，每五分钟一次
# 同时检测历史如果最近一条消息超过一小时，主动发起对话
def chat_thread(group_id: int):
    while True:
        # 获取最近聊天记录
        ...
        outer_memory = []
        for message in recent_history['data']:
            outer_memory.append({"role": model_name_transform(message["from_user"]["name"]), "text": message['chat_text']})
        res = model_chat(conversation_id, outer_memory=outer_memory)
        if res and isinstance(res, str):
            logger.info(f"model_chat: {res}")
            # 发送消息
            logger.info(f"send_message: {group_id}, {res}")
        time.sleep(300)


for group_id in group_id_list:
    # 新开线程监控group
    t = threading.Thread(target=chat_thread, args=(group_id,))
