...
def get_recent_history_within_timestamp(
    group_id: int | None = None, start_timestamp: int | None = None, end_timestamp: int | None = None
):
    ...
    while res["data"]:
        break_flag = False
        for i in res["data"]:
            if (not start_timestamp or i["timestamp"] >= start_timestamp) and (
                not end_timestamp or i["timestamp"] <= end_timestamp
            ):
                # print(i["timestamp"])
...
def send_chat_message(group_id: int, message: str | None = None, type: str = "txt", file_uuid: str | None = None):
    url = f"{app_endpoint}/api/sys/send_chat_message"

    if type == "img" and file_uuid:
        payload = json.dumps({"group_id": group_id, "type": type, "file_uuid": file_uuid})
    else:
...
if __name__ == "__main__":
    ...
    print(send_chat_message(316, "test", "txt"))
    pass
