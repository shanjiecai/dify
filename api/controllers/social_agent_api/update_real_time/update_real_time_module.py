...
from services.dataset_update_real_time_social_agent_service import DatasetUpdateRealTimeSocialAgentService
import datetime
...
def get_conversation_message_str(conversation_id: str = None, last_message_updated_at: datetime = None):
    conversation_messages_str = ""
    if conversation_id:
        messages = MessageService.pagination_by_more_than_updated_at(conversation_id=conversation_id, updated_at=last_message_updated_at)
        ...
        if messages:
            for message in messages:
                if message.query:
                    conversation_messages_str += message.role if message.role else "user" + ": " + message.query + "\n"
                if message.answer:
...
def save_conversation_to_file(conversation_id: str, app_id: str, conversation_messages_str: str):
    ...
    conversation_file_path = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'tmp',
                                          f"{current_date}_{conversation_id if conversation_id else app_id}.txt")
...
def update_dataset_id_with_app_id_pipeline(dataset_id: str, app_id: str = None, last_message_updated_at=None):
    dataset_update_real_time_social_agent = DatasetUpdateRealTimeSocialAgent.query.filter_by(dataset_id=dataset_id,
                                                                                             app_id=app_id).first()
    ...
    for conversation in conversations:
        message_str, last_message_id, last_message_updated_at = get_conversation_message_str(conversation.id,
                                                                                             last_message_updated_at=last_message_updated_at_new)
...
def init_dataset_update_real_time_social_agent(main_app: Flask):
    ...
    with main_app.app_context():
        try:
            if env == 'production' and mode == 'api':
                # if mode == 'api':
                dataset_upload_real_time_social_agent_list = DatasetUpdateRealTimeSocialAgentService.get_all_dataset_upload_real_time_social_agent()
            else:
    ...
    if dataset_upload_real_time_social_agent_list:
        for dataset_upload_real_time_social_agent in dataset_upload_real_time_social_agent_list:
            task = CountdownTask()
            t = threading.Thread(target=task.real_time_update, args=(dataset_upload_real_time_social_agent, main_app.app_context()))
            t.start()
            # 便于之后关闭线程
...
def restart_dataset_update_real_time_social_agent(main_app: Flask):
    ...
    with main_app.app_context():
        try:
            if env == 'production' and mode == 'api':
                # if mode == 'api':
                dataset_upload_real_time_list = DatasetUpdateRealTimeSocialAgentService.get_all_dataset_upload_real_time()
            else:
...
if __name__ == '__main__':
    ...
    db.session.commit()
    # update_dataset_id_with_app_id_pipeline("5974d4dd-c20c-4665-ac65-b47801e2e9ac", "a8e92867-2b58-4252-81d1-39473ad03d75")
    pass
